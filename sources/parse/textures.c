...
bool	last_textures_check(t_p_map *map, t_p_path *txtr, char *str, size_t i)
{
	while (i < ft_strlen(str) && ft_isspace(str[i]))
		i++;
	if (txtr->size == SPR_MAX && i == ft_strlen(str))
...
bool	are_textures_valid(t_p_map *map)
{
	...
	i = 0;
	map->fd = safe_open(map->path, map, -2);
